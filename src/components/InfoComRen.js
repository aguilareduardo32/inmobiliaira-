...
class InfoForms extends Component {
    constructor(props){
        super(props);
        this.state = { tipo: "", precioMax: "", formaDeCompra: "", zona: "", nombre: "", email: "", numero: "",  comentarios:""} 
    }
    ...
    handleFormSubmit = (event) => {
        ...
        axios.post("http://localhost:5000/api/infoCompra",{ tipo, precioMax, formaDeCompra, zona, nombre, email, numero,  comentarios })
        .then( () => { 
            ...
            <h3>Llene esta forma y uno de nuestros ejecutivos se comunicara con usted.</h3>
                <form class="info-form" onSubmit={this.handleFormSubmit}>
                            ...
                            <label>Delegación o Colonia de interes:</label>
                                        <input class="" type="string" name="zona"  value={this.state.zona} onChange={ e => this.handleChange(e)}></input>
                                             
                                            
                                            <label>Precio Max:</label>
                                        <input class="" type="string" name="precioMax"  value={this.state.precioMax} onChange={ e => this.handleChange(e)}></input>
                                        <label>Nombre:</label>
                                        <input class="" type="string" name="nombre"  value={this.state.nombre} onChange={ e => this.handleChange(e)}></input>
                                        
                                        <label>Telefono:</label>
                                        <input class="" type="number" name="numero" placeholder="NUMERO" value={this.state.numero} onChange={ e => this.handleChange(e)}></input> 
                                        
                                     
                            </div>   
                            <div className="column2">
                                        <label>Tipo:</label><br/>
                                            <select id="tipo" name="tipo" placeholder="Tipo" value={this.state.tipo} onChange={ e => this.handleChange(e)} >
                                                <option value='' disabled>Venta/Renta</option>
                                                <option tipo="venta" >VENTA</option>
                                                <option tipo="renta">RENTA</option>
                                            </select>
                                       <br/>
                                        <label>Forma de compra:</label><br/>
                                        <select id="formaDeCompra" name="formaDeCompra"  value={this.state.formaDeCompra} onChange={ e => this.handleChange(e)} >
                                        <option value='' disabled>contado/credito</option>
                                                <option tipo="credito" >credito</option>
                                                <option tipo="contado">contado</option>
                                            </select>
                                           <br/>
                                        <label>Email:</label>
                                        <input class="" type="string" name="email"  value={this.state.email} onChange={ e => this.handleChange(e)}></input>
